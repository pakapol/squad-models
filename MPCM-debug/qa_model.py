...
class QASystem(object):
    ...
    def setup_system(self):
        ...
        self.inner = inner = tf.matmul(self.p, qt)
        self.norms = norms = tf.matmul(tf.norm(self.p, axis=2, keep_dims=True), tf.norm(qt, axis=1, keep_dims=True))
        self.pf = p_filt = self.p * tf.reduce_max((inner + 1e-7) / (norms + 1e-7), axis=2, keep_dims=True)
    ...
    def optimize(self, sess, train_x, train_y):
        ...
        output_feed = [self.grads, self.loss, self.train_op]
        x, loss, _ = sess.run(output_feed, input_feed)
        for i in range(1,len(x)):
            print(x[i].shape, np.sum(np.isnan(x[i])),"\n")
        return loss
