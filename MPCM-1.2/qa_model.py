...
def iterate_across(padded_x, padded_y=None, batch_size=1, shuffle=True):
    """
...
class Encoder(object):
    ...
    def encode(self, inputs, masks, actual_len):
        ...
        with tf.variable_scope("encoder"):
            cell_fw = tf.nn.rnn_cell.GRUCell(self.state_size) # Change for tensorflow 1.0
            cell_bw = tf.nn.rnn_cell.GRUCell(self.state_size) # Change for tensorflow 1.0
...
class Matcher(object):
    ...
    def match(self, ps, p_finals, p_mask, qs, q_finals, q_mask):
        ...
        with tf.variable_scope("matcher"):
            ...
            return tf.nn.dropout(tf.concat_v2([match_fw, match_bw], axis=2), keep_prob=Config.dropout_keep_prob) # concat change for tensorflow 1.0
...
class Decoder(object):
    ...
    def decode(self, knowledge_rep, actual_len):
        ...
        with tf.variable_scope("decoder", initializer=tf.contrib.layers.xavier_initializer()):
            ...
            outputs = tf.nn.dropout(tf.concat_v2([out_fw, out_bw], axis=2), keep_prob=Config.dropout_keep_prob) # concat change for tensorflow 1.0
...
class QASystem(object):
    ...
    def setup_system(self):
        ...
        norms = tf.matmul(tf.sqrt(tf.reduce_sum(self.p*self.p, axis=2, keep_dims=True)+0.0001), tf.sqrt(tf.reduce_sum(qt*qt, axis=1, keep_dims=True)+.0001))
    ...
    def setup_loss(self):
        ...
        with tf.variable_scope("loss"):
            ...
            self.loss = tf.reduce_mean(loss_begin + loss_end) #+ Config.constraint_weight * tf.reduce_mean(tf.nn.relu(self.begin_score - self.end_score))
    ...
    def optimize(self, sess, train_x, train_y):
        ...
        # print(gr[0])
	#print("gradient = ",np.linalg.norm(gr[0]))
        return loss
    ...
    def answer_all(self, sess, test_x):
        ...
        all_a_s = []
        all_a_e = []
        for batch_x in iterate_across(padded_x=test_x, batch_size=Config.batch_size, shuffle=False):
    ...
    def validate(self, sess, valid_x, valid_y):
        ...
        valid_loss = 0
        num_iter = 0
        for batch_x, batch_y in iterate_across(valid_x, valid_y, Config.batch_size, shuffle=False):
    ...
    def evaluate_answer(self, sess, dataset, sample=100, log=False, mode="val"):
        ...
        # 1. Pad data
        p, q, span, pw, qw, ans = [[component[i] for i in indexer] for component in dataset[mode]]
    ...
    def train(self, sess, dataset, train_dir):
        ...
        # 1. Pad train data
        p, q, span, pw, qw, ans = dataset["train"]
        p, mask_p, actual_p = pad_input(p, Config.max_p_len)
        q, mask_q, actual_q = pad_input(q, Config.max_q_len)
        begin, end = zip(*span)
        # 2. Pad val data
        p_val, q_val, span_val, pw_val, qw_val, ans_val = dataset["val"]
